...
* 组件.vue后缀的文件中的name:'comp'能将vue工具中 将他命名为comp并且在缓存路由组件中 能够include=‘comp’此必须为组件名。
* 在路由器中配路由的时候 里面的name属性。在使用<router-link>的时候 。我们可以写：to=“{name}”此时将会将name转换城path地址。 此时to中必须写成对象形式
...
区别
>v-model（v-model:value） 双向绑定用于表单元素 作用在必须有value属性的元素上 所以向div、标题标签则无法成功绑定<br>v-bind  这个能将js的内容注入到前端页面视图 而修改前端内容js不受影响所以也称为单向绑定
> 代码
...
![在这里插入图片描述](/vue/vue-img/objectreturn.png)
...
![在这里插入图片描述](/vue/vue-img/vm%E5%AF%B9%E8%B1%A1.png)
...
>一、定义语法：
	...
	三、备注：
				 1.指令定义时不加v-，但使用时要加v-；
				 2.指令名如果是多个单词，要使用kebab-case命名方式，不要用camelCase命名。用驼峰的话会将userName自动转换城username导致出现异常。所以采用user-name
...
new Vue({
    ...
    directives: {
        // big函数何时会被调用？1.指令与元素成功绑定时（一上来）。2.指令所在的模板被重新解析时。但如果没有需要重新解析请求哪怕n值会变化 也不会调用此指令
...
2. **created（创建后）**：实例创建完成，实例上配置的 options 包括 data、computed、watch、methods 等都配置完成，但是此时渲染得节点还未挂载到 DOM，所以不能访问到 `$el`属性。d
...
1. 技术经验总结 ：1.1 当父组件给子组件传递数据或者函数时可以通过propr进行传递 。1.2当子组件给父组件传递数据时可以采取先用父组件传一个函数再有子组件调用进行回调函数（优先采取自定义事件来处理）。1.3 当父组件给孙子组件或者孙子组件给父组件传递信息都可以采取全局总线（注意  如果组件销毁请将将总线中得方法也一起用$off释放） 1.4 当兄弟组件传递数据时 只能用数据总线来进行传递。
...
3. 其中的this的指向问题。此时如果写成普通的函数 因为此function回调是由vue进行回调的所以此时的this指向vue或vc组件。采用箭头函数将返回出去。外面包含的也同为vue或vc组件。
...
1. 概念：当state中的数据需要经过加工后再使用时，可以使用getters加工。相当于在vue中的计算属性的能力 通过监控vuex中state对象的属性修改。来对创建的值进行修改
...
> 备注：mapActions与mapMutations使用时，若需要传递参数需要：在模板中绑定事件时传递好参数，否则传的参数是事件对象(event)。就是在绑定或者使用此事件的地方如@click=‘test(n)’ 需要在此时调用数据。个人觉得在数值方面会快捷很多。对于函数并不建议（简单的除外）
...
1. 如果想实现父路由器套子路由器的话必须采用多级路由来进行书写 。如果是同级路由将会覆盖调父组件。
...
4. query参数和params参数的区别：
   ...
   他们本质上是前端vue在页面中跳转传参的两种不同方式 。query参数通过？id=666拼接请求路径来获取参数。（vue采取$route.query.id来获取参数。）params通过在路由器里面通过/home/ap/:id通过告诉路由器那些是路径那是参数来进行传参。（vue采取$route.params.id来获取参数） 
   
   并且params是在路由器里面地址栏上面修改不会随着刷新丢失。而query携带的参数会被刷新导致变成特定的字符串。
